    ...
    if (file) {
      ...
      if (file.data.length > 100 * 1024 * 1024) {
        if (callback)
          return callback({
            status: "error",
            message: "File quá lớn! Giới hạn 100MB.",
          });
        return;
      }
  ...
  try {
    ...
    if (!message.groupID) {
      io.to(message.senderID)
        .to(seenUserID)
        .emit("updateSingleChatSeenStatus", messageID);
...
socket.on("roleSwitched", ({ userID, targetUserID, groupID }) => {
  console.log(
    `Quyền LEADER đã được chuyển từ ${userID} sang ${targetUserID} trong nhóm ${groupID}`
  );
...
socket.on("groupRenamed", ({ newGroupName, groupID }) => {
  console.log(
    `Tên nhóm đã được thay đổi thành ${newGroupName} trong nhóm ${groupID}`
  );
  // Cập nhật UI hoặc thực hiện các thao tác khác trong ứng dụng
});
```
...
- **Mô tả:**
  ...
  **Lưu ý:** _Chỉ phía web client (trình duyệt) nhận sự kiện và xử lý đăng nhập._
